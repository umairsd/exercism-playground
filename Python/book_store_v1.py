...
def total(basket):
    ...
    for combo in list_of_sets:
        total_price = 0
        for book_set in combo:
            price = len(book_set) * price_of_one_book * (
                        100 - discount_percentage.get(len(book_set), 0))
            total_price += price
...
def build_all_sets(book_count_tuples, list_of_sets, current_set):
    ...
    for max_set_length in range(max_possible, 0, -1):
        ...
        # recurse.
        build_all_sets(sorted_tuples, list_of_sets, updated_set)
